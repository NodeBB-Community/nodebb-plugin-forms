	...
	window.ACPForms = {
		load: function () {
			...
			var $pluginForms = $('#plugin-forms'),
				...
				formElements = {
					'text': {
						...
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									<br><input data-name="name" data-value="default" class="pfa-input" name="{name}" type="text" value="{default}"/>'
					},
					'textarea': {
						display: 'Text Area',
						camel: 'TextArea',
						...
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									<br><textarea data-name="name" data-text="default" class="pfa-input" name="{name}">{default}</textarea>'
					},
					'number': {
						display: 'Number',
						camel: 'Number',
						...
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									<br><input data-name="name" data-value="default" class="pfa-input" name="{name}" type="number" value="{default}"/>'
					},
					'radiogroup': {
						...
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									...
									<div class="radio" data-object="options">\
										<input data-checked="default" data-value="value" class="pfa-input-option" type="radio" value="{options.value}" checked="{options.default}" name="{name}" data-name="{name}"/>\
					...
					'checkboxes': {
						...
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									...
									<div class="checkbox" data-object="options">\
										<input data-checked="default" data-value="value" class="pfa-input-option" type="checkbox" value="{options.value}" checked="{options.default}" name="{name} data-name="{name}""/>\
										<label data-text="label" class="control-label pfa-input-option-label" tabindex="0">{options.label}</label>\
									</div>\
									<!-- END options -->',
						newdata: {options: [{value: '1', label: 'Option 1'},{value: '2', label: 'Option 2'},{value: '3', label: 'Option 3'}]}
					...
					'select': {
						...
						description: '',
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									<select data-object="options" data-name="name" class="pfa-input" name="{name}">\
									<!-- BEGIN options -->\
									<option data-value="value" options-selected="default" data-text="label" class="pfa-input-option" value="{options.value}" <!-- IF options.default -->selected="selected"<!-- ENDIF options.default -->>{options.label}</option>\
									<!-- END options -->',
						newdata: {options: [{value: '1', label: 'Option 1'},{value: '2', label: 'Option 2'},{value: '3', label: 'Option 3'}]}
					},
					'selectmultiple': {
						...
						description: '',
						template:	'<label data-text="label" class="control-label" tabindex="0">{label}</label>\
									<select data-object="options" data-name="name" class="pfa-input" name="{name}" multiple>\
									<!-- BEGIN options -->\
									<option data-value="value" options-selected="default" data-text="label" class="pfa-input-option" value="{options.value}" <!-- IF options.default -->selected="selected"<!-- ENDIF options.default -->>{options.label}</option>\
									<!-- END options -->',
						newdata: {options: [{value: '1', label: 'Option 1'},{value: '2', label: 'Option 2'},{value: '3', label: 'Option 3'}]}
					...
					'buttons': {
						...
						template: '<d<div data-container class="btn-group"><!-- BEGIN buttons --><button data-object="buttons" data-type="type" data-text="label" type="{buttons.type}">{buttons.label}</button><!-- END buttons --></div>',
					...
					'info': {
						...
						template:	'<label data-text="label" class="control-label h3" tabindex="0">{label}</label>\
									<br><textarea data-text="default" data-name="name" class="pfa-input" name="{name}">{default}</textarea>'
					},
					'divider': {
						display: 'Divider',
						camel: 'Divider',
				...
				modalInputTemplate = '<div class="form-group"><label class="col-sm-4 control-label">{label}</label><div class="col-sm-8"><input type="text" data-klass="{klass}" {klass}="{key}" value="{value}"></div></div>',
				modalInputArrayTemplate = '<div class="pfa-modal-input-array" data-object="{key}">\
			...
			var addInput = function(inputSortable, type, data) {
				...
				data.label = data.label || ( formElement.display || 'Unknown Input' ) + ' Label';
				data['default'] = data['default'] || '';

				var typeSingle = formElement.single || type,
			...
			$pluginForms.on('click', '.pfa-btn-edit-input', function (e) {
				...
				function makeModalInput(klass, key, value) {
				...
				$openInput.find('>[data-value], >[data-container]>[data-value]:not([data-object], [data-object]>[data-value]), >[data-text], >[data-container]>[data-text]:not([data-object]), >[data-type], >[data-container]>[data-type]:not([data-object]), [data-name], >[data-object], >[data-container]>[data-object]').each(function(){
					if (this.hasAttribute('data-object')) {
						var key = this.getAttribute('data-object'),
							$this = $(this),
							obj = { },
							$arr;

						$arr = $modalInputContent.find('[data-object="'+ key +'"]');
						if (!$arr.length) {
						...
						$this.children('[data-value]').addBack('[data-value]').each(function(){
							obj[this.getAttribute('data-value')] = this.value;
						});
						$this.children('[data-text]').addBack('[data-text]').each(function(){
							obj[this.getAttribute('data-text')] = this.innerHTML;
						});

						$arr.append('<div class="pfa-modal-input-array-element form-group">\
										<div class="col-md-4"><input class="pfa-option-value" value="'+ obj.value +'"/></div>\
										<div class="col-md-4"><input class="pfa-option-label" value="'+ obj.label +'"/></div>\
										<div class="col-md-4">\
						...
						if (this.hasAttribute('data-value')) {
							makeModalInput('data-value', this.getAttribute('data-value'), this.value);
						}
						if (this.hasAttribute('data-text')) {
							makeModalInput('data-text', this.getAttribute('data-text'), this.innerHTML);
						}
						if (this.hasAttribute('data-type')) {
							makeModalInput('data-type', this.getAttribute('data-type'), this.getAttribute('type'));
						}
					}
					if (this.hasAttribute('data-name') && !$modalInputContent.find('[data-klass="data-name"]').length) {
						$modalInputContent.prepend(modalInputTemplate
							.replace(/{klass}/g, 'data-name')
							.replace('{key}', this.getAttribute('data-name'))
							.replace('{value}', this.getAttribute('name'))
							.replace('{label}', this.getAttribute('data-name'))
						);
					}
				});
				...
				$modalInputContent.find('.form-group>div>[data-klass]').each(function(){
					var $this = $(this),
						klass = $this.attr('data-klass'),
						value = $this.val(),
						key = $this.attr(klass),
						$el = $openInput.find('['+ klass +'="'+ key +'"]');

					obj[key] = value;
				});

				$modalInputContent.find('[data-object]').each(function(){
					var $this = $(this),
						label = $this.attr('data-klass'),
						value = $this.val(),
						key = $this.attr(klass),
						$el = $openInput.find('['+ klass +'="'+ key +'"]');
				...
				$openInput.append(templates.parse(formElements[type].template || formElements.text, obj));
		...
		setObjectKey: function (_obj, key, val) {
			var part, obj, parts;
		...
		getObjectFromInput: function ($inputPanel) {
			var type, input;

			type = $inputPanel.attr('type');
			input = {
				type: type,
				label: $inputPanel.find('label[data-text]').text() !== 'empty' ? $inputPanel.find('label[data-text]').text() : '',
				name: $inputPanel.find('.pfa-input').attr('name') || $inputPanel.find('label[data-text]').text().trim().toLowerCase().replace(/ /g, '-'),
				require: $inputPanel.find('[data-require]').data('require')
			};
			input['is' + (formElements[type] ? formElements[type].camel : 'Text')] = true;

			$inputPanel.find('>[data-value], >[data-container]>[data-value]:not([data-object])').each(function(){
				ACPForms.setObjectKey(input, this.getAttribute('data-value'), this.value);
			});
			$inputPanel.find('>[data-text], >[data-container]>[data-text]:not([data-object])').each(function(){
				ACPForms.setObjectKey(input, this.getAttribute('data-text'), this.innerHTML);
			});
			$inputPanel.find('>[data-type], >[data-container]>[data-type]:not([data-object])').each(function(){
				ACPForms.setObjectKey(input, this.getAttribute('data-type'), this.getAttribute('type'));
			});
			$inputPanel.find('>[data-name], >[data-container]>[data-name]:not([data-object])').each(function(){
				ACPForms.setObjectKey(input, this.getAttribute('data-name'), this.getAttribute('name'));
			});
			$inputPanel.find('>[data-checked], >[data-container]>[data-checked]:not([data-object])').each(function(){
				ACPForms.setObjectKey(input, this.getAttribute('data-checked'), this.checked);
			});
			$inputPanel.find('>[data-object], >[data-container]>[data-object]').each(function(){
				...
				// $this.children('[data-name]').addBack('[data-name]').each(function(){
					// obj[this.getAttribute('data-name')] = this.getAttribute('name');
				// });
