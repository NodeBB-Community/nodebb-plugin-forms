	...
	window.ACPForms = {
		load: function () {
				...
				formElements = {
					'text': {
						...
						template:	'<div class="form-group">\
										<label data-text="label" class="control-label pfa-input-label" tabindex="0">{label}</label>\
					...
					'hidden': {
						...
						newdata: {label:'Hidden Input', 'default':''}
					},
					'buttons': {
						type: 'buttons',
						display: 'Buttons',
						...
						template: '<div class="form-group btn-group"><!-- BEGIN buttons --><button data-object=\'{"buttons":{"data-type":"type","data-text":"label"}}\' type="{buttons.type}">{buttons.label}</button><!-- END buttons --></div>',
					...
					'sendusers': {
						...
						template:	'<div class="pfa-jumbotron">\
										...
										<input data-value="users" value="{value}">\
									</div>'
					},
					'validate': {
			...
			var addInput = function(inputSortable, type, data) {
				...
				var stamp = Date.now(),
					formElement = formElements[type] || formElement.text;
				data = data || formElement.newdata || { };
				data.name = data.name || type + stamp;
				data.label = data.label || 'Text Label';
				...
				$('#save').click( function (event) {
					...
					$('#pfa-forms-list').children().each(function(){
						...
						$(this).find('.pfa-input-panel').each(function(){
							...
							input = {
								type: type,
								label: $inputPanel.find('.pfa-input-label').text() !== 'empty' ? $inputPanel.find('.pfa-input-label').text() : '',
								name: $inputPanel.find('.pfa-input').attr('name') || $inputPanel.find('.pfa-input-label').text().trim().toLowerCase().replace(/ /g, '-'),
							...
							switch (type) {
								...
								case 'checkboxes':
								case 'radiogroup':
									var $options = $inputPanel.find(".pfa-input-option");
									var $labels = $inputPanel.find(".pfa-input-option-label");
							...
							$inputPanel.find('[data-text]').each(function(){
								ACPForms.set(input, this.getAttribute('data-text'), this.innerHTML);
							});
							$inputPanel.find('[data-type]').each(function(){
								ACPForms.set(input, this.getAttribute('data-type'), this.getAttribute('type'));
							});
							$inputPanel.find('[data-name]').each(function(){
								ACPForms.set(input, this.getAttribute('data-name'), this.getAttribute('name'));
							});
							$inputPanel.find('[data-object]').each(function(){
								var arrs = $(this).data('object');

								for (var arr in arrs) {
									if (!Array.isArray(input[arr])) input[arr] = [];

									var i = input[arr].push({ })-1,
										obj = input[arr][i];

									for (var prop in arrs[arr]) {
										switch (prop) {
											case 'data-value':
												obj[arrs[arr][prop]] = this.value;
												break;
											case 'data-name':
												obj[arrs[arr][prop]] = this.getAttribute('name');
												break;
											case 'data-type':
												obj[arrs[arr][prop]] = this.getAttribute('type');
												break;
											case 'data-text':
												obj[arrs[arr][prop]] = this.innerHTML;
												break;
											default:
												break;
										}
